...
def edit(request, employee_code):
    if request.method == 'GET':
        employee_details = EmployeeManagement.objects.get(employee_code=employee_code)
        show = {
...
def upload_file(request):
    if "GET" == request.method:
        ...
        return render(request, 'show_emp.html', context={'employee_detail': employee_detail})

def search(request, employee_code):
    return EmployeeManagement.objects.filter(employee_code)
